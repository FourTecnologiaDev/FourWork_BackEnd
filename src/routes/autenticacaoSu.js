...
router.post("/autenticacaoSu", async function (req, res) {
    try{
      ...
      if(req.body.email && req.body.password)
        retorno = await crud('SuporteUsuario', req.body, 'authenticate');
        if(retorno)
          authenticate = (retorno.length == 0 ? false : true)

      // Autentica com a criptografia
      retorno = await crud('SuporteUsuario', {email: req.body.email}, 'find');
      if(retorno)
